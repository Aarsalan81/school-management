...
export default function EditCourse(props) {
    const { editCourse, setEditCourse, item, setIsSuccess} = props;
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            tutorialID:item.tutorialName ?? null,
            teacherID:item.teacherName ?? null,
            startDate:item.startDate ?? "" , 
            startTime:item.startTime ?? null, 
            endTime:item.endTime ?? null,
            dayType:item.dayType ?? 1,
        },
        onSubmit: (values) => {
            const newValue = {
                ...values,
            ...
            axios.put(
                ...
                .then(function (response) {
                    ...
                    setAlertMassage("اطلاعات دوره با موفقیت ویرایش گردید")
                    setEditCourse(false)
                    setIsSuccess(true)
                }).catch(function (error) {
    ...
    return (
        <>
            <Dialog maxWidth="sm" open={editCourse} onClose={() => setEditCourse(false)}>
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <DialogActions>
                        <Button sx={{ fontFamily: "medium" }} variant='contained' color='inherit' onClick={() => setEditCourse(false)}>انصراف</Button>
